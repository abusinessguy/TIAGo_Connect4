...
def image_callback(msg):
    try:
        ...
        tag_centers, _ = detect_aruco_tags(gray_frame)

	rospy.loginfo(f"Detected {len(tag_centers)}")
        if tag_centers and len(tag_centers) == 4:
            rospy.loginfo("Detected 4 ArUco Tags")
...
def detect_aruco_tags(image, reduction=0.5):
    """Detect ArUco Tags in the given image."""
    preprocessed_image = preprocess_image(image, reduction)
    corners, ids, rejected = cv2.aruco.detectMarkers(preprocessed_image, aruco_dict, parameters=parameters)
